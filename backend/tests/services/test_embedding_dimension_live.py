...
tests/nlp/test_embeddings.py's real-model version guarded before the ML
split; the check itself moved here because agreement can now only be
verified against a real, reachable inference service - the fake/stub
used in tests/nlp/test_embeddings.py can't assert anything about what
the real model actually outputs.
