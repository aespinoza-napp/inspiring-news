...
2. **Real enrichment/fact-check pipeline (under active development, not yet wired to the API).**
   `src/workflows/enrichment.py` (`NewsEnrichmentPipeline`) runs the real NLP stack in `src/processors/nlp/` (keywords, entities, claims, topic classifier, sentiment, quality, embeddings — see `src/processors/nlp/processor.py`) to build a `src/models/core/enriched_article.py` (`EnrichedArticle`). This is validated and fact-checked by `src/services/fact_checker/`:
...
`src/models/` is organized into subpackages by domain rather than one flat directory: `core/` (`claim`, `news`, `source`, `enriched_article`, `similarity` — types used across multiple subsystems), `nlp/` (`quality`, `sentiment_result`, `topic_prediction`, `nlp_result`, `topics` — enrichment output shapes), `scraper/` (`extraction`, `search_query`, `search_result`), `fact_checker/` (`duplicate_result`, `evidence`, `fact_check`, `fact_check_report`, `validation_result` — mirrors `src/services/fact_checker/`'s own layout). No `__init__.py` files anywhere in `src/` — everything is a namespace package, consistent throughout.
...
`tests/scraper/` no longer touches the network — `Scraper`, `ExtractorService`, and `RSSDiscoveryStrategy` all have proper mocked/fixture-based coverage, and `tests/builders/source_builder.py`'s `build_source()` is the shared `NewsSource` factory for scraper tests. **Do not run the full suite unfiltered anyway**: `tests/nlp/test_all_news.py` depends on `backend/data/raw` already being populated with real scraped articles (nothing in the current suite populates it). A handful of other tests are pre-existing failures unrelated to any single feature (`tests/test_connection.py` needs a running Neo4j; `tests/test_repository.py`, `tests/test_sources.py`, `tests/test_fact_checker.py`, and `tests/nlp/{test_claims,test_classifier,test_pipeline}.py` fail against current `main` regardless of what you're working on — verify against a clean checkout before assuming you broke something). `tests/test_api.py` and `tests/test_pipeline.py` fail to even import (`NLPProcessor` isn't exported from `src.processors.nlp`). When verifying a change, prefer a targeted `uv run pytest tests/<area>` over a bare `uv run pytest`.
